%% Perform Reliability of ITPS and ICPS in increasing numbers of trials
% This script performs the reliability of ITPS and ICPS analyses by 
%increasing numbers of trials, showed in Morales et al. (under review).
...
Save_Data = '/data/rosalind/Dropboxes/moraless/ECHO/Reliability/R1_subs_300_100/';

% Data_Location = '/export/data/cdl/Projects/ECHO/CSD_tranformed_data/Zoo/';
% Save_Data = '/export/data/cdl/Projects/ECHO/CSD_TimeFreqs/Zoo/';
    ...
    if exist([subject '_TimeFreqs_ITPS_ICPS_Reliability.mat'], 'file') == 2
        ...
        if TrialNums(sub).Correct > cutoff && TrialNums(sub).Error > cutoff
            ...
            for chanj=1:size(correct_phase_data, 1)
                ...
                seed = 1; % change this electrode number depending what seed you want to use - NOTE this is based on the small channels
                ...
                for subsample_i=1:length(subsample_array)
                    ...
                    if Cond1Num >= subsample_n 
                        for samp=1:subsample_num
                            ...
                            correct_subtrials = randsample(TrialNums(sub).Correct,subsample_n,false);
                            
                            % Divide them into half
                            correct_subtrials_1 = correct_subtrials(1:ceil(length(correct_subtrials)/2));
                            correct_subtrials_2 = correct_subtrials(ceil(length(correct_subtrials)/2) + 1: end);
                            ...
                            correct_ICPS_subsamples(subsample_i, 1,samp,:,:) = correct_phase_connectivity_temp_1;
                            correct_ICPS_subsamples(subsample_i, 2,samp,:,:) = correct_phase_connectivity_temp_2;
                            
                            %create matrix of subsamples for ITPS - half x samp x freq x time
                            correct_ITPS_subsamples(subsample_i, 1,samp,:,:) = correct_ITPS_temp_1;
                            correct_ITPS_subsamples(subsample_i, 2,samp,:,:) = correct_ITPS_temp_2;
                        end
                    end
                    ...
                    if Cond2Num >= subsample_n 
                        for samp=1:subsample_num
                            %subsampling the crossspecden - with replacement across subsample, but without replacement within subsamples
                            %Get indices of trials for this subsample
                            error_subtrials = randsample(TrialNums(sub).Error,subsample_n,false);
                            
                            % Divide them into half
                            error_subtrials_1 = error_subtrials(1:ceil(length(error_subtrials)/2));
                            error_subtrials_2 = error_subtrials(ceil(length(error_subtrials)/2) + 1: end);
                            ...
                            %create matrix of subsamples for ICPS - n_subsamples x half x samp x freq x time
                            error_ICPS_subsamples(subsample_i, 1,samp,:,:) = error_phase_connectivity_temp_1;
                            error_ICPS_subsamples(subsample_i, 2,samp,:,:) = error_phase_connectivity_temp_2;
                            
                            %create matrix of subsamples for ITPS - n_subsamples x half x samp x freq x time
                            error_ITPS_subsamples(subsample_i, 1,samp,:,:) = error_ITPS_temp_1;
                            error_ITPS_subsamples(subsample_i, 2,samp,:,:) = error_ITPS_temp_2;
                        end
                    end
            ...
            for chanj=1:size(correct_ITPS_subsamp_nobaseCorr,1)
                for subsample_i=1:size(correct_ITPS_subsamp_nobaseCorr,2)
                    for half_i=1:2
                        for samp_i=1:size(correct_ITPS_subsamp_nobaseCorr,4) 
                            for fi = 1:size(correct_ITPS_subsamp_nobaseCorr,5)
                                correct_ITPS_subsamp_baseCorr(chanj,subsample_i,half_i,samp_i,fi,:) = ( correct_ITPS_subsamp_nobaseCorr(chanj,subsample_i,half_i,samp_i,fi,:) - mean(correct_ITPS_subsamp_nobaseCorr(chanj,subsample_i,half_i,samp_i,fi,basetimeidx(1):basetimeidx(end))));
                                correct_icps_subsamp_baseCorr(chanj,subsample_i,half_i,samp_i,fi,:) = ( correct_ICPS_data(chanj,subsample_i,half_i,samp_i,fi,:) - mean(correct_ICPS_data(chanj,subsample_i,half_i,samp_i,fi,basetimeidx(1):basetimeidx(end))));
                            end
                        end
                    end
                end
            ...
            for chanj=1:size(error_ICPS_data,1)
                for subsample_i=1:size(error_ICPS_data,2)
                    for half_i=1:2
                        for samp_i=1:size(correct_ITPS_subsamp_nobaseCorr,4)
                            for fi = 1:size(correct_ITPS_subsamp_nobaseCorr,5)
                                error_ITPS_subsamp_baseCorr(chanj,subsample_i,half_i,samp_i,fi,:) = ( error_ITPS_subsamp_nobaseCorr(chanj,subsample_i,half_i,samp_i,fi,:) - mean(error_ITPS_subsamp_nobaseCorr(chanj,subsample_i,half_i,samp_i,fi,basetimeidx(1):basetimeidx(end))));
                                error_icps_subsamp_baseCorr(chanj,subsample_i,half_i,samp_i,fi,:) = ( error_ICPS_data(chanj,subsample_i,half_i,samp_i,fi,:) - mean(error_ICPS_data(chanj,subsample_i,half_i,samp_i,fi,basetimeidx(1):basetimeidx(end))));
                            end
                        end
                    end
                end
            ...
            %% Commenting this out because this is now done in a separate script rather than using trial-by-trial TF data
...
%                 'correct_ITPS_data', 'error_ITPS_data', 'correct_icps_data', 'error_icps_data');
            ...
            correct_ITPS_data_4_8 = reshape(mean(correct_ITPS_data(:,:,:,:,Freqs_Idx(1):Freqs_Idx(2),:),5),sz(dims));
            sz = size(error_ITPS_data);
            error_ITPS_data_4_8 = reshape(mean(error_ITPS_data(:,:,:,:,Freqs_Idx(1):Freqs_Idx(2),:),5),sz(dims));
            
            sz = size(correct_icps_data);
            correct_icps_data_4_8 = reshape(mean(correct_icps_data(:,:,:,:,Freqs_Idx(1):Freqs_Idx(2),:),5),sz(dims));
            sz = size(error_icps_data);
            error_icps_data_4_8 = reshape(mean(error_icps_data(:,:,:,:,Freqs_Idx(1):Freqs_Idx(2),:),5),sz(dims));
            ...
            dims = [1 2 3 4 6];
            sz = size(correct_ITPS_data);
            correct_ITPS_data_1_4 = reshape(mean(correct_ITPS_data(:,:,:,:,Freqs_Idx(1):Freqs_Idx(2),:),5),sz(dims));
            sz = size(error_ITPS_data);
            error_ITPS_data_1_4 = reshape(mean(error_ITPS_data(:,:,:,:,Freqs_Idx(1):Freqs_Idx(2),:),5),sz(dims));
            
            sz = size(correct_icps_data);
            correct_icps_data_1_4 = reshape(mean(correct_icps_data(:,:,:,:,Freqs_Idx(1):Freqs_Idx(2),:),5),sz(dims));
            sz = size(error_icps_data);
            error_icps_data_1_4 = reshape(mean(error_icps_data(:,:,:,:,Freqs_Idx(1):Freqs_Idx(2),:),5),sz(dims));
...
%                 'correct_tf_trials_data_1_4', 'error_tf_trials_data_1_4', 'channel_location');
            ...
            correct_ITPS_data_4_8 = reshape(mean(correct_ITPS_data_4_8(:,:,:,:,Times_Idx(1):Times_Idx(2)),5),sz(dims));
            correct_ITPS_data_1_4 = reshape(mean(correct_ITPS_data_1_4(:,:,:,:,Times_Idx(1):Times_Idx(2)),5),sz(dims));
            sz = size(error_ITPS_data_4_8);
            error_ITPS_data_4_8 = reshape(mean(error_ITPS_data_4_8(:,:,:,:,Times_Idx(1):Times_Idx(2)),5),sz(dims));
            error_ITPS_data_1_4 = reshape(mean(error_ITPS_data_1_4(:,:,:,:,Times_Idx(1):Times_Idx(2)),5),sz(dims));
            
            sz = size(correct_icps_data_4_8);
            correct_icps_data_4_8 = reshape(mean(correct_icps_data_4_8(:,:,:,:,Times_Idx(1):Times_Idx(2)),5),sz(dims));
            correct_icps_data_1_4 = reshape(mean(correct_icps_data_1_4(:,:,:,:,Times_Idx(1):Times_Idx(2)),5),sz(dims));
            sz = size(error_icps_data_4_8);
            error_icps_data_4_8 = reshape(mean(error_icps_data_4_8(:,:,:,:,Times_Idx(1):Times_Idx(2)),5),sz(dims));
            error_icps_data_1_4 = reshape(mean(error_icps_data_1_4(:,:,:,:,Times_Idx(1):Times_Idx(2)),5),sz(dims));
